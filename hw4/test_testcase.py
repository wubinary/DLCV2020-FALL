from p1.model import Conv4
from p1.utils import distance_metric 
from p2.model import Conv4_Haullu
#from p2.utils import distance_metric 
...
def predict(args, model, data_loader):
    ...
    with torch.no_grad():
        ...
        for i, (data, target) in enumerate(data_loader):
            ...
            proto_latent = support_latent.reshape(
                        args.N_way,args.N_shot,-1).mean(dim=1)

            # TODO: classify the query data depending on the its distense with each prototype
            logits = distance(query_latent, proto_latent)
...
if __name__=='__main__':
    ...
    df = pd.read_csv(str(args.testcase_csv).replace('.csv','_gt.csv'))
    df = pd.DataFrame(prediction_results, columns=df.columns)
