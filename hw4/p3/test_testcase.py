...
def predict(args, model, data_loader):
    ...
    with torch.no_grad():
        ...
        for i, (data, target) in enumerate(data_loader):
            ...
            # query 
            query = query_latent.reshape(1,args.N_query*args.N_way,-1)

            #query_hallu = model.hallucinate(query.squeeze(0), args.hallu_m)

            #query_aug = torch.cat((query,query_hallu),dim=0) #(1+m,query*way,z)
            #query_aug = query_aug.view((1+args.hallu_m)*args.N_query*args.N_way,-1)
            query_aug = query.squeeze(0)

            # distances
            logits = distance(query_aug, proto_aug)
            #logits = logits.reshape((1+args.hallu_m),
            #                    args.N_query*args.N_way,args.N_way)
