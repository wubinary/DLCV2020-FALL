...
def train(args, dataloaders):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for i in range(len_dataloader):
            ...
            domain_label = torch.zeros(src_imgs.size(0)+tar_imgs.size(0), 1).to(args.device)
...
def valid(args, dataloaders, models=None):
    ...
    if models is None:
        load = torch.load(
                f"./result/3_2/{args.source}2{args.target}/best_model.pth")
        ...
        domain_classifier.load_state_dict(load['D'])
        domain_classifier.to(args.device)
    else:
        feature_extractor = models['F']
        label_predictor = models['C']
        domain_classifier = models['D']
    ...
    for i,(imgs, lbls) in enumerate(valid_source_dataloader):
        ...
        print(f"\t [{i+1}/{len(valid_source_dataloader)}]", end="  \r")
    ...
    for i,(imgs, lbls) in enumerate(valid_target_dataloader):
        ...
        tar_accs.append(acc*bsize)

        count += bsize 

        print(f"\t [{i+1}/{len(valid_target_dataloader)}]", end="  \r")

    mean_src_acc, mean_tar_acc = mean_(src_accs), sum(tar_accs)/count
...
if __name__=='__main__':
    ...
    wandb.init(config=args, 
        project=f'dlcv_dann_{args.source}2{args.target}')
